...
async def cmd_backup(msg: Message):
    """Create backup of database and shorteners configuration (admin/owner only)."""
    if msg.from_user is None:
        return
    ...
    user = await get_user_by_telegram_id(msg.from_user.id)
    if not user or not is_allowed_to_create(user):
        await msg.reply("❌ Hanya admin/owner yang dapat membuat backup.")
        return

    try:
        # Show progress
        status_msg = await msg.reply("🔄 <b>Membuat backup...</b>\n\n<i>Mohon tunggu, proses ini mungkin memakan waktu beberapa detik.</i>", parse_mode="HTML")

        # Create database backup
        db_dump_path = await backup_database()

        # Create shorteners backup
        shorteners_path = backup_shorteners()

        # Create ZIP file
        zip_path = create_backup_zip(db_dump_path, shorteners_path)
        ...
        with open(zip_path, 'rb') as backup_file:
            await msg.reply_document(
                ...
                caption="✅ <b>Backup berhasil dibuat!</b>\n\n"
                       f"📁 <b>Nama file:</b> <code>{os.path.basename(zip_path)}</code>\n"
                       f"📊 <b>Ukuran:</b> {os.path.getsize(zip_path) / 1024:.1f} KB\n\n"
                       f"<i>Simpan file ini di tempat yang aman untuk restore di masa depan.</i>",
                parse_mode="HTML"
            )
        ...
        await msg.reply(f"❌ <b>Gagal membuat backup:</b> {e}", parse_mode="HTML")


@router.message(Command("restore"))
async def cmd_restore(msg: Message, state: FSMContext):
    """Restore database and shorteners from backup ZIP file (admin/owner only)."""
    if msg.from_user is None:
        return
    ...
    user = await get_user_by_telegram_id(msg.from_user.id)
    if not user or not is_allowed_to_create(user):
        await msg.reply("❌ Hanya admin/owner yang dapat melakukan restore.")
        return
    ...
    try:
        ...
        text = (
            "📤 <b>Mode Restore Aktif</b>\n\n"
            "Kirim file backup ZIP yang ingin direstore.\n\n"
            "<b>⚠️ PERINGATAN:</b>\n"
            "• Data saat ini akan <b>DIGANTI</b> dengan data dari backup\n"
            "• Pastikan file backup valid dan dari sumber terpercaya\n"
            "• Proses ini <b>TIDAK BISA DIBATALKAN</b>\n\n"
            f"<i>Kirim file ZIP atau ketik /cancel untuk membatalkan.</i>"
        )

        await msg.reply(text, parse_mode="HTML", reply_markup=back_to_main_buttons())

    except Exception as e:
        logger.exception("Failed to start restore mode: %s", e)
        await state.clear()
        await msg.reply(f"❌ <b>Gagal memulai mode restore:</b> {e}", parse_mode="HTML")
...
async def handle_restore_file(msg: Message, state: FSMContext, bot: Bot):
    ...
    try:
        # Check if user is still admin/owner
        user = await get_user_by_telegram_id(msg.from_user.id)
        if not user or not is_allowed_to_create(user):
            await state.clear()
        ...
        if not msg.document.file_name or not msg.document.file_name.lower().endswith('.zip'):
            await msg.reply("❌ <b>File tidak valid!</b>\n\n<i>Harap kirim file backup dengan format ZIP.</i>", parse_mode="HTML")
            return

        # Show progress
        status_msg = await msg.reply("🔄 <b>Memproses file backup...</b>\n\n<i>Mohon tunggu, sedang mendownload dan memverifikasi file.</i>", parse_mode="HTML")

        # Download the file
        file_path = os.path.join(tempfile.gettempdir(), f"restore_{msg.document.file_name}")
        await bot.download(msg.document, destination=file_path)

        await status_msg.edit_text("🔄 <b>Mengekstrak backup...</b>\n\n<i>Mohon tunggu, sedang mengekstrak file backup.</i>", parse_mode="HTML")

        # Extract backup
        extracted_files = extract_backup_zip(file_path)

        if 'database_backup.sql' not in extracted_files or 'shorteners_backup.json' not in extracted_files:
            await status_msg.edit_text("❌ <b>File backup tidak valid!</b>\n\n<i>File ZIP harus berisi database_backup.sql dan shorteners_backup.json.</i>", parse_mode="HTML")
            os.unlink(file_path)
            shutil.rmtree(os.path.dirname(extracted_files['database_backup.sql']))
            await state.clear()
            return

        await status_msg.edit_text("🔄 <b>Merestore database...</b>\n\n<i>Mohon tunggu, sedang merestore database dari backup.</i>", parse_mode="HTML")

        # Restore database
        db_stats = await restore_database(extracted_files['database_backup.sql'])

        await status_msg.edit_text("🔄 <b>Merestore konfigurasi shorteners...</b>\n\n<i>Mohon tunggu, sedang merestore pengaturan shorteners.</i>", parse_mode="HTML")

        # Restore shorteners
        shorteners_restored = restore_shorteners(extracted_files['shorteners_backup.json'])
        ...
        success_text = (
            ...
            f"📊 <b>Database:</b>\n"
            f"   ├ Tables created: {db_stats.get('tables_created', 0)}\n"
            f"   └ Rows inserted: {db_stats.get('rows_inserted', 0)}\n\n"
            f"🔗 <b>Shorteners:</b> {'✅ Restored' if shorteners_restored else '❌ Failed'}\n\n"
            f"<i>Bot telah direstart dengan data dari backup. Silakan test fungsionalitasnya.</i>"
        )
        ...
        await state.clear()
        await msg.reply(f"❌ <b>Gagal melakukan restore:</b> {e}", parse_mode="HTML")


@router.message(RestoreStates.waiting_for_file, Command("cancel"))
async def cancel_restore(msg: Message, state: FSMContext):
    """Cancel restore operation."""
    await state.clear()
    await msg.reply("❌ <b>Restore dibatalkan.</b>\n\n<i>Kembali ke menu utama.</i>", reply_markup=back_to_main_buttons(), parse_mode="HTML")